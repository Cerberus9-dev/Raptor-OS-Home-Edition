...
Browser choice — Firefox (pre-installed, no download), Brave (~120 MB), or Chrome (~150 MB). Shows a network check, download progress bar, and retry prompt on failure. Closing the dialog keeps Firefox.
App picker — 65+ optional apps across 12 categories. Nothing is pre-ticked. Everything here is also installable later from Discover or flatpak install flathub <id>. "Skip — Install Nothing" is always available.
What's Included
